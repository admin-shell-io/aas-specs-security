...
* changed: DateTime and Time literals in access rules now use explicit patterns based on AAS core meta `MatchesXsDateTime` and `MatchesXsTime`, which are based on XML Schema `xs:dateTime`/ISO 8601 lexical forms. Values require seconds, may include fractional seconds and timezone offsets, and incomplete values such as `09:00` need to be completed before serialization, e.g. `09:00:00Z`. [#63](https://github.com/admin-shell-io/aas-specs-security/issues/63)
