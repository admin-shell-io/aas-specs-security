...
Resuable parts are defined at the beginning of <AllAccessPermissionRules> as DEFATTRIBUTES, DEFOBJECTS, DEFACLS and DEFFORMULAS. (@todo do we want to add <AllAccessPermissionDefs> to the BNF grammar?).
...
|DEFATTRIBUTES
|Pre-defined <AttributeGroup>. This <AttributeGroup> can be referenced using <UseAttributeGroup>.

|DEFACLS
|Pre-defined <ACL>. This ACL can be referenced using <UseACL>.

|DEFOBJECTS
|Pre-defined <ObjectGroup>. This <ObjectGroup> can be referenced using <UseObjectGroup>.

|DEFFORMULAS
|Pre-defined formula. This <Formula> can be referenced using <UseFormula>. Formulas define a Boolean result to decide whether an access rule is (@todo)applicable.
|===
...
<AccessPermissionRule> (@todo) defines a single access control rule granting permissions on the designated AAS objects (@todo). Table 4Table 4 describes the details of <AccessPermissionRule>. 
