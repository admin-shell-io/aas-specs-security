...
Reusable parts are defined in <AllAccessPermissionDefs> as DEFATTRIBUTES, DEFOBJECTS, DEFACLS and DEFFORMULAS.
...
A pre-defined <AttributeGroup> can be referenced using <UseAttributeGroup>.
A pre-defined <ObjectGroup> can be referenced using <UseObjectGroup>.
...
A pre-defined <ACL> can be referenced using <UseACL>.
...
FORMULAs define a Boolean result to decide whether an access rule is applicable.
A pre-defined <Formula> can be referenced using <UseFormula>.
...
<AccessPermissionRule> defines a single access control rule granting permissions on the designated AAS objects. Table 4 describes the details of <AccessPermissionRule>. 
