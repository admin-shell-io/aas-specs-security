...
The introduction in xref:introduction.adoc[Introduction] has explained in detail the background for AAS Security.
...
If access control is enabled for an AAS interface, access shall only be granted if there is (at least) one access control rule which permits the requested operation on the designated <<objects,AAS object>>.
...
|`"CLAIM"`
|ABAC attribute of the subject/requestor provided as a claim in the access token to the AAS interface (see xref:introduction.adoc#_access_token[Access token]).
