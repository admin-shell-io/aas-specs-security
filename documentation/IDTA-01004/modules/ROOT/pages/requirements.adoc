...
xref:access-rule-model.adoc#attributes[Attributes] describes an approach to provide information for authorization via ABAC attributes in an authenticated way.
...
* Assets
* AAS objects (xref:access-rule-model.adoc#objects[Objects])
...
Details on how this requirement is to be met by the information model are outlined in xref:access-rule-model.adoc#access-rule-model[Access Rule Model].
...
a| Any attempt of unauthorized access or modifiy the AAS (<<_authorization_enforcement>>). The event SHALL contain information on the origin of the unauthorized access (e.g., network source, subject name) and on the target AAS object (xref:access-rule-model.adoc#objects[Objects]).
...
* Which AAS object (xref:access-rule-model.adoc#objects[Objects]) is affected
...
| Update of information of an existing AAS object (xref:access-rule-model.adoc#objects[Objects]).
...
* Authentication and authorization services used for the realization of an AAS (<<_identification_and_authentication_of_aas_user_application_users>>, <<_identification_and_authentication_of_aas_user_applications_and_aas_interface>>, xref:access-rule-model.adoc#attributes[Attributes]).
...
The discovery, exploration, and use of Submodels is addressed by <<_authorization_enforcement>> and xref:access-rule-model.adoc#access-rule-model[Access Rule Model].
