...
Important authentication flows are explained in this clause in 4.13.
...
(Reproduced by permission of DKE, German Member of IEC (https://www.dke.de/de[www.dke.de]).
For the valid edition see the latest publication of IEC or DIN EN IEC and https://www.dke.de/de[www.dke.de] and https://www.vde-verlag.de/[www.vde-verlag.de])
...
* Type 1 is the exchange as an AASX file link:#bib3[[3\]] (part 5).
...
* Type 2 is the access to AAS by the API link:#bib2[[2\]] (part 2) to a server.
...
<<image-access-aas-servers>> shows an example of an User Application accessing 3 AAS Servers and a Registry.
...
image::access-aas-servers.png[width=575,height=492]
...
In that case the Access Rules define the access to the descriptors, i.e. by rules with semantiId, assetId or ID for identifiable.
...
image::aas-server-implementation.png[width=543,height=873]
...
image::aas-registry-implementation.png[width=642,height=861]
...
In general, the ABAC request flow is described in link:#bib8[[8\]].
Originally, ABAC relies upon the data-flow model and language model of the OASIS eXtensible Access Control Markup Language (XACML) specifications link:#bib9[[9\]].
...
For many internal Use Cases the security may be completey company specific not using the standardized AAS security.
...
image::auth-flow-oauth.png[width=525,height=279]
...
It is commonly an alpha numeric string or Base64 encoded string.
...
The Resource server evaluates and checks the claims in the access token and validates it with the Acess Rules, as specified later in this specification.
...
image::auth-flow-openid.png[width=614,height=355]
...
According to the Dataspace Protocol (DSP) link:#bib7[[7\]], a Dataspace is “A set of technical services that facilitate interoperable Dataset sharing between entities.”
...
image::auth-flow-dataspace.png[width=532,height=343]
...
The introduction in Chapter 4 has explained in detail the background for AAS Security.The use of Identity Providers and Authentication Flows have been shown, which provide an access token to the client user application.
...
Figure XX gives an overview of the Access Rule Model.
Section xx defines the text serialization of the Access Rule Model.
Section xx defines the JSON schema of the Access Rule Model.
...
A formula can include nested boolean expressions, comparions and string operations.
...
In addition to the text below, the further details of the formula expressions are explained in https://admin-shell-io.github.io/aas-specs-antora/IDTA-01002/v3.1/general.html#_query_language .

.Overview Access Rule Model
[[image-access-rule-model]]
image::access-rule-model.png[width=642,height=554]
...
The grammar includes whitespaces, so that the examples in Annex B can be validated.
...
An important special operation is $match, which can be used with any element containing a list of elements, e.g. semanticId[], specificAssetId[], SubmodelElementList or SubmodelElelementCollection.
...
Such references use the Part 1 Mapping in Section “Text Serialization of Values of Type Reference” for specific instances of elements, e.g. an AAS, a Submodel, a ConceptDescription or a specific SubmodelElement.
See: https://admin-shell-io.github.io/aas-specs-antora/IDTA-01001/v3.1/mappings/mappings.html#reference-serialization
...
To define all descpriptors “(aasDesc)*” or “(smdesc)*” may be used in DescriptorLiterals accordingly.
...
The oprator can copy and paste the access rules into his AAS implementation and only needs to make adjustments.
...
Version 3.1 of the AAS API defines queries and uses the same grammar and same JSON schema to define queries.
...
For more information see Wikipediafootnote:[https://en.wikipedia.org/wiki/Backus%E2%80%93Naur_form].
...
[#bib5]
[5] file:///C:/Users/sbader/AppData/Local/Temp/Tom%20Preston-Werner[Tom Preston-Werner].
Semantic Versioning.
